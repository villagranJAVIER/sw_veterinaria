<template>
    <app-layout title="Agregar reservación">
      <template #header>
              <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                  {{ titulo }}
              </h2>
          </template>
  
          <div class="py-12">
              <div class="max-w-screen-2xl mx-auto sm:px-6 lg:px-8 text-left text-2xl">
  
                  <div class="bg-gray-50 shadow-2xl p-8">
                    <form class="p-10 flex flex-col items-center" @submit.prevent="guardar">
                        
                        <jet-label for="nombre_c" value="Nombre del usuario:"/>
                        <jet-input
                            id="nombre_c"
                            class="px-5 py-3 w-3/4 border border-gray-600 rounded"
                            v-model="form.nombre_c"
                            :class="{ 'is-invalid': form.errors.nombre_c }"
                            required
                            placeholder="Nombre del cliente"
                        />
                        <jet-input-error :message="form.errors.nombre_c" />
  
                        <jet-label for="nombre_a" value="Nombre de la mascota:" class="mt-8"/>
                        <jet-input
                            id="nombre_a"
                            class="px-5 py-3 w-3/4 border border-gray-600 rounded"
                            v-model="form.nombre_a"
                            :class="{ 'is-invalid': form.errors.nombre_a }"
                            required
                            placeholder="Nombre de la mascota"
                        />
                        <jet-input-error :message="form.errors.nombre_a" />
  
                        <jet-label for="edad_a" value="Edad de la mascota:" class="mt-8"/>
                        <jet-input
                            id="edad_a"
                            class="px-5 py-3 w-3/4 border border-gray-600 rounded"
                            v-model="form.edad_a"
                            :class="{ 'is-invalid': form.errors.edad_a }"
                            required
                            placeholder="Edad de la mascota"
                        />
                        <jet-input-error :message="form.errors.edad_a" />

                        <jet-label for="peso_a" value="Peso de la mascota(KG):" class="mt-8"/>
                        <jet-input
                            id="peso_a"
                            class="px-5 py-3 w-3/4 border border-gray-600 rounded"
                            v-model="form.peso_a"
                            :class="{ 'is-invalid': form.errors.peso_a }"
                            required
                            placeholder="Peso de la mascota"
                        />
                        <jet-input-error :message="form.errors.peso_a" />

                        <jet-label for="raza_a" value="Raza de la mascota:" class="mt-8"/>
                        <jet-input
                            id="raza_a"
                            class="px-5 py-3 w-3/4 border border-gray-600 rounded"
                            v-model="form.raza_a"
                            :class="{ 'is-invalid': form.errors.raza_a }"
                            required
                            placeholder="Raza de la mascota"
                        />
                        <jet-input-error :message="form.errors.raza_a" />
                        
                        <jet-label for="service_id" value="Servicio solicitado:" class="mt-8"/>
                        <span
                            id="service_id"
                            class="px-5 py-3 w-3/4 border border-gray-600 rounded">
                            {{ servicio.nombre }}
                        </span>

                        <div class="flex justify-around mt-10 text-2xl mb-5 w-3/5">
                            <Link
                                :href="route(`${routeName}index`)"
                                class="underline text-gray-600 hover:text-gray-900"
                                >
                                Cancelar
                            </Link>
  
                            <jet-button
                                @click="guardar"
                                class="ml-4 text-xl"
                                :class="{ 'text-white': form.processing }"
                                :disabled="form.processing"
                                >
                                <span class="animate-spin mr-1" v-show="form.processing">&#9696;</span> Guardar
                            </jet-button>
                        </div> 
                                    
                    </form>
                </div>
  
            </div>
      </div>
    </app-layout>
  </template>
  
  <script>
  import AppLayout from "@/Layouts/AppLayout.vue";
  import { Link } from "@inertiajs/inertia-vue3";
  import JetLabel from "@/Components/Label.vue";
  import JetInput from "@/Components/Input.vue";
  import JetInputError from "@/Components/InputError.vue";
  import JetButton from "@/Components/Button.vue";
  import { useForm } from "@inertiajs/inertia-vue3";
  
  export default {
  name: "Create",
  props: {
    titulo: { type: String, required: true },
    routeName: { type: String, required: true },
    servicio: { type: Object, required: true},
  },
  components: {
    AppLayout,
    Link,
    JetLabel,
    JetInput,
    JetInputError,
    JetButton,
  },
  setup(props) {
    const form = useForm({ nombre_c: "", nombre_a: "", edad_a: "", peso_a: "", raza_a: "", service_id: "", user_id: ""});
    const guardar = () => {
        form.service_id = props.servicio.id;
        form.post(route("reservaciones.store"));
    };
  
    return { form, guardar };
  },
  
  };
  </script>
  